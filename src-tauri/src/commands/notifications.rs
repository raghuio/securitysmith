...
pub fn do_get_notifications(conn: &Connection) -> Result<Vec<Notification>, String> {
    ...
    let rows = stmt
        ...
        .map_err(|e| format!("DB: {}", e))?;
    for row in rows {
        let (id, client_name, status) = row.map_err(|e| format!("Row: {}", e))?;
    ...
    let rows = stmt
        ...
        .map_err(|e| format!("DB: {}", e))?;
    for row in rows {
        let (id, title, engagement_name, client_name) = row.map_err(|e| format!("Row: {}", e))?;
...
pub fn do_dismiss_notification(conn: &Connection, key: &str) -> Result<(), String> {
    conn.execute(
...
pub fn do_get_notification_count(conn: &Connection) -> Result<u32, String> {
...
mod tests {
    ...
    fn make_finding_with_deadline(conn: &Connection, eid: u32, days_ago: i64) -> u32 {
        ...
        let deadline = format!("now-{} days", days_ago);
        conn.execute(
            ...
            params![eid, endpoint, format!("-{} days", days_ago)],
        )
        .unwrap();
        conn.last_insert_rowid() as u32
    }
...
pub fn dismiss_notification(state: State<AppState>, key: String) -> Result<(), String> {
    ...
    do_dismiss_notification(conn, &key)
}
...
pub fn get_notification_count(state: State<AppState>) -> Result<u32, String> {
    let vault = state
        ...
        .map_err(|_| "State poisoned".to_string())?;
    let conn = vault.as_ref().ok_or("Vault not unlocked.")?;
    do_get_notification_count(conn)
}
