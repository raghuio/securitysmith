...
pub fn create_export(
    ...
    let file = std::fs::File::create(&save_path).map_err(|e| format!("File: {}", e))?;
...
pub fn create_encrypted_export(
    ...
    std::fs::write(&save_path, &encrypted).map_err(|e| format!("File: {}", e))?;
...
pub fn is_import_encrypted(file_path: String) -> Result<bool, String> {
    let data = std::fs::read(&file_path).map_err(|e| format!("File: {}", e))?;
...
pub fn decrypt_import_to_temp(file_path: String, password: String) -> Result<String, String> {
    ...
    let tmp =
        std::env::temp_dir().join(format!("ss_import_{}.zip", chrono::Utc::now().timestamp()));
    std::fs::write(&tmp, &plaintext).map_err(|e| format!("Write: {}", e))?;
...
pub fn preview_import(state: State<AppState>, file_path: String) -> Result<ImportPreview, String> {
    ...
    let file = std::fs::File::open(&file_path).map_err(|e| format!("File: {}", e))?;
    let mut archive =
        zip::ZipArchive::new(std::io::BufReader::new(file)).map_err(|e| format!("ZIP: {}", e))?;
    ...
    let manifest: serde_json::Value =
        serde_json::from_str(&manifest_raw).map_err(|e| format!("Parse: {}", e))?;
    ...
    if let Ok(mut clients_file) = archive.by_name("clients.json") {
        ...
        if let Ok(clients) = serde_json::from_str::<Vec<serde_json::Value>>(&clients_raw) {
            for client in clients {
                ...
                ).map_err(|e| format!("Insert client: {}", e))?;
    ...
    if let Ok(mut engagements_file) = archive.by_name("engagements.json") {
        ...
        if let Ok(items) = serde_json::from_str::<Vec<serde_json::Value>>(&raw) {
            for item in items {
                ...
                ).map_err(|e| format!("Insert engagement: {}", e))?;
                let new_id = conn.last_insert_rowid();
