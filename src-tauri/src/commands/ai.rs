use crate::commands::{clients, engagements, findings};
...
You have access to the following tools. Respond with a JSON object when you want to use a tool:
{"tool": "create_client", "args": {"name": "..."}}
...
pub async fn ai_chat(
    ...
    // Stream the response token-by-token (word-by-word) via Tauri events
    ...
    let _ = app.emit("ai_stream_done", serde_json::json!({"full": &message}));
...
fn execute_tool(
    ...
    match tool_name {
        "create_client" => {
            ...
            Ok(format!("Created client '{}' with id {}", name, id))
        }
        "create_engagement" => {
            let client_id = args
                .get("client_id")
            ...
            Ok(format!("Created engagement '{}' with id {}", name, id))
        }
        "create_finding" => {
            ...
            Ok(format!("Created finding '{}' with id {}", title, id))
        }
        "search_clients" => {
