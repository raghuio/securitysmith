...
pub fn do_create_retest_engagement(
    ...
    let (client_id, orig_name, _client_name, target_area, assessment_kind) =
        original.ok_or_else(|| "Original engagement not found.".to_string())?;
    ...
    .map_err(|e| format!("Failed to create retest engagement: {}", e))?;
    ...
    for row in rows {
        let (
            ...
            tags,
            notes,
        ) = row.map_err(|e| format!("Row parse failed: {}", e))?;
        conn.execute(
        ...
        .map_err(|e| format!("Failed to clone finding: {}", e))?;
    }
...
pub fn do_bulk_update_finding_status(
    ...
    if !valid.contains(&client_response) {
        return Err(format!("Invalid client_response: {}", client_response));
    }
    for id in finding_ids {
        ...
        .map_err(|e| format!("Failed to update finding {}: {}", id, e))?;
    }
    Ok(())
}

pub fn do_get_overdue_findings(conn: &Connection) -> Result<Vec<serde_json::Value>, String> {
    let mut stmt = conn
        .prepare(
...
pub fn create_retest_engagement(
    ...
    do_create_retest_engagement(conn, original_engagement_id)
}
...
pub fn list_retest_engagements(
    ...
    do_list_retest_engagements(conn, original_engagement_id)
}
...
pub fn get_retest_comparison(
    ...
    let vault = state
        ...
        .map_err(|_| "State poisoned".to_string())?;
    let conn = vault.as_ref().ok_or("Vault not unlocked.")?;
    do_get_retest_comparison(conn, retest_engagement_id)
}
...
pub fn bulk_update_finding_status(
    ...
    let mut vault = state
        ...
        .map_err(|_| "State poisoned".to_string())?;
    let conn = vault.as_mut().ok_or("Vault not unlocked.")?;
    do_bulk_update_finding_status(conn, &finding_ids, &client_response)
}
