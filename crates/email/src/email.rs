...
fn validate_email_field(value: &str, field: &str) -> Result<(), String> {
    if value.contains('\r') || value.contains('\n') {
        return Err(format!("{} contains invalid newline characters", field));
    }
    Ok(())
}
...
pub async fn send_email(
    ...
    validate_email_field(&to, "to")?;
    validate_email_field(&subject, "subject")?;
...

    {
        ...
        conn.execute(
            ...
            params!["system", "EMAIL_SENT", 0, "", "", format!("to={to}, subject={subject}, attachments={attachment_count}, client_id={client_id:?}, engagement_id={engagement_id:?}")],
        )
...
pub fn get_follow_up_reminders(state: State<AppState>) -> Result<Vec<FollowUpReminder>, String> {
    ...
    let rows = stmt
        ...
        .filter_map(|r| match r {
            Ok(v) => Some(v),
            Err(e) => {
                eprintln!("Warning: reminder row skipped: {e}");
                None
            }
        });
...
pub async fn send_test_email(
...

    {
        ...
        conn.execute(
            "INSERT INTO audit_log (table_name, action, record_id, old_value, new_value, context) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params!["system", "EMAIL_SENT", 0, "", "", format!("to={to}, subject={subject}")],
        )
        .map_err(|e| format!("Audit failed: {e}"))?;
    }
