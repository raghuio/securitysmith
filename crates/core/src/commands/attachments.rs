...
fn validate_attachment(input: &AttachmentInput) -> crate::error::Result<()> {
    if !matches!(
...
fn do_upload_attachment(
    ...
    if decoded.is_empty() {
        return Err(AppError::Generic("Empty file data.".to_string()));
    }
    const MAX_SIZE: usize = 50 * 1024 * 1024; // 50MB
    if decoded.len() > MAX_SIZE {
        return Err(AppError::Generic("File exceeds 50MB limit.".to_string()));
    }
...
fn do_get_attachment(conn: &Connection, id: u32) -> crate::error::Result<Attachment> {
    let item: Option<Attachment> = conn
        ...
        .optional()
        .map_err(AppError::from)?;
    item.ok_or(AppError::Generic("Attachment not found.".to_string()))
}
...
fn do_list_attachments(
    ...
    entity_type: &str,
    entity_id: u32,
) -> crate::error::Result<Vec<Attachment>> {
    let mut stmt = conn
        .prepare(
            "SELECT id, entity_type, entity_id, filename, original_name, mime_type, file_size, sha256, sort_order, created_at
...
fn do_delete_attachment(conn: &Connection, data_dir: &Path, id: u32) -> crate::error::Result<()> {
...
fn do_rename_attachment(conn: &Connection, id: u32, new_name: &str) -> crate::error::Result<()> {
...
pub fn upload_attachment(
    ...
    do_upload_attachment(conn, &data_dir, &input).map_err(|e| e.to_string())
}
...
pub fn list_attachments(
    ...
    do_list_attachments(conn, &entity_type, entity_id).map_err(|e| e.to_string())
}
...
pub fn delete_attachment(
    ...
    let conn = vault.connection().map_err(|e| e.to_string())?;
    let data_dir = get_data_dir(&app_handle)?;
    do_delete_attachment(conn, &data_dir, id).map_err(|e| e.to_string())
}
...
pub fn rename_attachment(state: State<AppState>, id: u32, new_name: String) -> Result<(), String> {
    let mut vault = state
        ...
        .map_err(|_| "Internal state error".to_string())?;
    let conn = vault.connection().map_err(|e| e.to_string())?;
    do_rename_attachment(conn, id, &new_name).map_err(|e| e.to_string())
}
...
pub fn reorder_attachments(
    ...
    let mut vault = state
        ...
        .map_err(|_| "Internal state error".to_string())?;
    let conn = vault.connection().map_err(|e| e.to_string())?;
    do_reorder_attachments(conn, &entity_type, entity_id, &ordered_ids).map_err(|e| e.to_string())
}
...
pub fn read_attachment_file(
    ...
    if !matches!(entity_type.as_str(), "finding" | "engagement" | "document") {
    ...
    let guard = state
        ...
        .lock()
        .map_err(|_| "Internal state error".to_string())?;
    let _conn = guard.connection_ref().map_err(|e| e.to_string())?;
    let data_dir = get_data_dir(&app_handle)?;
    let safe_filename = sanitize_filename(&filename);
    let bytes = do_read_attachment_file(&data_dir, &entity_type, entity_id, &safe_filename)?;
...
fn generate_thumbnail(data: &[u8], max_dim: u32) -> Result<Vec<u8>, String> {
    let img = image::load_from_memory(data).map_err(AppError::from)?;
    let w = img.width();
    let h = img.height();
