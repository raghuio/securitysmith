...
fn do_check_duplicates(
    ...
    for row in rows {
        let (title, eps_str) = row.map_err(AppError::from)?;
...
fn do_parse_file(
    ...
    let meta = std::fs::metadata(file_path).map_err(AppError::from)?;
    if meta.len() == 0 {
    ...
    let content = std::fs::read(file_path).map_err(AppError::from)?;
    ...
    let mut findings: Vec<ParsedFinding> = match format {
        ...
        "csv" => {
            ...
            CsvParser::new(mapping).parse(&content)
        }
...
"#;
    ...
    fn test_csv_parsing() {
        ...
        let findings = CsvParser::new(mapping).parse(csv.as_bytes()).unwrap();
