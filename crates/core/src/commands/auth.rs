use crate::crypto;
use crate::db;
use crate::state::{AppState, RecoveryState, VaultState};
use rusqlite::params;
...
pub async fn create_vault(
    ...
    *vault = VaultState::Unlocked { conn, key };
...
impl std::fmt::Display for ValidationResult {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.success {
            write!(f, "Recovery phrase validated successfully")
        } else {
            write!(f, "Validation failed; new phrase generated")
        }
    }
}
...
#[tauri::command]
pub async fn validate_recovery_words(
    app_handle: AppHandle,
    state: State<'_, AppState>,
    phrase: String,
    positions: Vec<u8>,
    ...
    let vault_key = {
        let guard = state
            ...
            .lock()
            .map_err(|_| "Internal state error".to_string())?;
        *guard.key().map_err(|e| e.to_string())?
    };
...
pub async fn recover_vault(
    ...
    let normalized = match bip39::Mnemonic::parse_in(bip39::Language::English, phrase.trim()) {
        Ok(m) => m.to_string(),
        Err(_) => return Err("Invalid recovery phrase.".to_string()),
    };

    if !crypto::is_valid_recovery_phrase(&normalized) {
    ...
    let (envelope, recovery_salt) = match db::load_recovery_envelope(&data_dir) {
        Ok(v) => v,
        Err(e) => {
            state.record_failed_attempt();
            return Err(format!("Failed to load recovery envelope: {e}"));
        }
    };

    let recovery_key = match crypto::derive_recovery_key_async(normalized, recovery_salt).await {
        Ok(k) => k,
        Err(e) => {
            state.record_failed_attempt();
            return Err(format!("Recovery key derivation failed: {e}"));
        }
    };

    let vault_key = match crypto::decrypt_envelope(&envelope, &recovery_key) {
        Ok(k) => k,
        Err(_) => {
            state.record_failed_attempt();
            return Err("Incorrect recovery phrase. Please try again.".to_string());
        }
    };
...
#[tauri::command]
pub async fn rotate_recovery_phrase(
    app_handle: AppHandle,
    state: State<'_, AppState>,
    password: String,
) -> Result<RecoveryState, String> {
...
pub async fn change_master_password(
    ...
    let mut vault_guard = state
        ...
        .lock()
        .map_err(|_| "Internal state error".to_string())?;
    *vault_guard = VaultState::Unlocked { conn, key: new_key };
